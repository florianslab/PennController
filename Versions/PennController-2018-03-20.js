...
// - Version alpha 0.2
// - Last Update:  2018, March 20
// - Note: initial release (alpha)
...
(function(){
    ...
    class ImageInstr extends Instruction {
        constructor(image) {
            ...
            if (image != Abort) {
                ...
                this.setElement($("<div>").css("display", "inline-block"));
    ...
    // Adds a Canva where you can place multiple instructions
    // Done immediately
    class CanvaInstr extends Instruction {
        constructor(w,h) {
            super({width: w, height: h}, "canva");
            if (w != Abort) {
                if (typeof(w) != "number" || typeof(h) != "number" || w < 0 || h < 0)
                    return Abort;
                let element = $("<div>").css({width: w, height: h, position: "relative"}).addClass("PennController-Canva");
        ...
        run() {
            ...
            for (let o in this.objects) {
                ...
                if (!(origin instanceof Instruction)) {
                    console.log("Warning: element #"+o+" of canva is not a proper instruction; ignoring it.");
                    continue;
                }
        ...
        // Adds an object onto the canva at (X,Y) on the Z-index level
        _addObject(instruction, x, y, z) {
            ...
            // If instruction has not been run yet, but if CANVA has been run: run instruction
        ...
        // Returns an instruction to add/update an instruction on the canva at (X,Y)
        // Done immediately
    ...
    PennController.instruction.image = function(image){ return new ImageInstr(image); };
    ...
    PennController.instruction.canva = function(width, height){ return new CanvaInstr(width, height); };
